...
${new Date().toLocaleDateString()}`,
        ...
        // 重新设计的文案，突出太空旅行和通信延迟的调侃
        `【银河系游戏监管委员会】

${playerName}玩家，

我们的深空追踪系统显示，您已经"跑路"到【${celestialBody.name}】去了！

距离地球${actualDistance}？真是个绝佳的躲避游戏的地方！

您的【${gameName}】队友们已经等了${formatTime(waitTime)}，他们现在确信您是故意跑到宇宙边缘去逃避比赛的。

由于物理定律限制，您的任何辩解都需要${formatTime(waitTime)}才能传回地球。

建议：要么立即返回，要么承认您就是个"宇宙级鸽王"。
...
您在【${celestialBody.name}】下的订单："紧急！请立即送游戏手柄到我的位置！"

我们想说：兄弟，您跑到距离地球${actualDistance}的地方，然后抱怨配送慢？

就算我们的快递员开光速飞船，也需要${formatTime(waitTime)}才能到达您的位置！

下次想玩【${gameName}】的时候，请不要跑到宇宙边缘再下单，谢谢！
...
您的投诉："网络延迟${formatTime(waitTime)}，【${gameName}】根本没法玩！这是什么垃圾网络？"

经检测，您目前位于【${celestialBody.name}】，距离地球${actualDistance}。

亲，这不是网络问题，这是您跑得太远的问题！

光速就这么快，物理定律我们也改不了。建议您：
1. 搬回地球
2. 或者等我们发明超光速网络
3. 再或者您发明时空穿越技术
...
您拨打我们的心理热线说："我在【${celestialBody.name}】很孤独，朋友们都不理我了。"

经过分析，我们发现问题所在：

您跑到距离地球${actualDistance}的地方，然后抱怨朋友们不回消息？

您的【${gameName}】朋友们其实一直在回复，只是消息需要${formatTime(waitTime)}才能传到您那里！

建议：下次想要社交的时候，请不要跑到宇宙边缘。
...
您的投诉："【${celestialBody.name}】的WiFi太差了！完全连不上【${gameName}】！"

我们想提醒您：这里距离地球${actualDistance}，您期望什么样的网络质量？

您已经在那里"度假"${formatTime(waitTime)}了，是时候面对现实：
...
您在【${celestialBody.name}】点的外卖："一份炸鸡，一台游戏机，要快！我要玩【${gameName}】！"

配送距离：${actualDistance}
预计送达时间：${formatTime(waitTime)}

亲，您这是在为难我们的配送员啊！

就算骑最快的光速摩托车，也需要这么长时间。而且到了您那里，炸鸡早就凉透了，游戏机可能都过时了。

建议：下次想吃外卖玩游戏，请不要跑到宇宙边缘。
...
您的求助："我在【${celestialBody.name}】直播，为什么观众都说我卡顿？"

亲爱的，您距离地球${actualDistance}，直播信号需要${formatTime(waitTime)}才能传回来！

观众看到的不是直播，是"延播"！

当您说"大家好"的时候，地球上的【${gameName}】比赛可能都结束了。

建议：
1. 回地球直播
2. 或者改做"宇宙延时摄影"
3. 标题改成"${formatTime(waitTime)}前的我"
...
function generateAndShowPostcard() {
    ...
    try {
        // 随机选择一个模板
        const templateIndex = Math.floor(Math.random() * 24);
